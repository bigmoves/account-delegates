...
- `transition:*` scopes are dropped from a delegated session along with everything but `repo:`, `blob:`, and `space:`. A client that only knows the transitional scopes, as the Bluesky app does today, cannot operate a delegated session until it asks for granular ones. `space:` permissions survive narrowing only when the delegate holds them verbatim.
